...
class KDEModel(Model):
    ...
    def __call__(self, data, smallest_N=None, data_prior=None):#, proc_idx=None, return_dict=None):
        ...
        #(SC) FIX ME: make inference in log likelihood
        ...
        for idx, (obs, d_pdf) in enumerate(zip(np.atleast_3d(data),data_prior)):
            ...
            likelihood_per_samp = np.log(self.pdf(obs)) 
            ...
            if smallest_N is not None:
                ...
                likelihood_per_samp = logsumexp([q_weight + likelihood_per_samp, pi_reg*np.ones(likelihood_per_samp.shape)], axis=0)
            likelihood_per_samp -= np.log(d_pdf)
            likelihood[idx] = logsumexp([likelihood[idx], logsumexp(likelihood_per_samp) - np.log(data.shape[1])], axis=0)
